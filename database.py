...
def seed_default_users(password_hash_fn):
    ...
    if not cursor.execute('SELECT id FROM users WHERE email = ?', ('student@tawun.com',)).fetchone():
        cursor.execute(
            ...
            ('أحمد الطالب', 'student@tawun.com', default_password, '0501234567', 'student', 'كلية الاتصال والإعلام')
        )
        user_id = cursor.lastrowid
        cursor.execute(
            'INSERT INTO students (user_id, gender, major, age, skills, course_name, crn, section_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            (user_id, 'male', 'الصحافة والإعلام الرقمي', 22, 'كتابة صحفية، وسائل التواصل', 'مقدمة في الصحافة الرقمية', 'CS101', '01')
        )
    ...
    if not company_user:
        cursor.execute(
            'INSERT INTO users (name, email, password, phone, role, department) VALUES (?, ?, ?, ?, ?, ?)',
            ('شركة/جهة حكومية التدريبية', 'company@tawun.com', default_password, '0507654321', 'company', 'الصحافة والإعلام الرقمي')
        )
        user_id = cursor.lastrowid
        cursor.execute(
    ...
    if not cursor.execute('SELECT id FROM users WHERE email = ?', ('supervisor@tawun.com',)).fetchone():
        cursor.execute(
            ...
            ('د. محمد المشرف', 'supervisor@tawun.com', default_password, '', 'supervisor', 'كلية الاتصال والإعلام', 'male')
        )
        user_id = cursor.lastrowid
        cursor.execute(
            'INSERT INTO supervisors (user_id, department) VALUES (?, ?)',
            (user_id, 'كلية الاتصال والإعلام')
        )
    ...
    if not cursor.execute('SELECT id FROM users WHERE email = ?', ('supervisor2@tawun.com',)).fetchone():
        cursor.execute(
            'INSERT INTO users (name, email, password, phone, role, department, gender) VALUES (?, ?, ?, ?, ?, ?, ?)',
            ('د. رانية المشرفة', 'supervisor2@tawun.com', default_password, '', 'supervisor', 'كلية الاتصال والإعلام', 'female')
        )
        user_id = cursor.lastrowid
        cursor.execute(
            'INSERT INTO supervisors (user_id, department) VALUES (?, ?)',
...
def seed_default_applicants(cursor, password_hash_fn):
    ...
    college_dept = 'كلية الاتصال والإعلام'
    extra_students = [
        ('سارة أحمد', 'sara@tawun.com', '0501112233', 'female', college_dept, 'الإنتاج المرئي والمسموع', 21, 'مونتاج، تصوير'),
        ('خالد العتيبي', 'khalid@tawun.com', '0502223344', 'male', college_dept, 'الاتصال التسويقي', 23, 'حملات، محتوى'),
        ('نورة السعيد', 'noura@tawun.com', '0503334455', 'female', college_dept, 'العلاقات العامة', 20, 'فعاليات، تواصل'),
        ('عمر الشمري', 'omar@tawun.com', '0504445566', 'male', college_dept, 'الصحافة والإعلام الرقمي', 22, 'صحافة، تحرير'),
        ('فاطمة القحطاني', 'fatima@tawun.com', '0505556677', 'female', college_dept, 'الإنتاج المرئي والمسموع', 21, 'إخراج، صوت'),
    ]
